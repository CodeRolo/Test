Shader "Abandoned Manor Exterior/RimDetailMask" {
Properties {
	_MainTex ("Base (RGB) Gloss (A)", 2D) = "white" {}
	_SecTex ("Secondary (RGB) Gloss (A)", 2D) = "white" {}
	_AOTex ("AO (R) DetailAO (G) Mask (B)", 2D) = "white" {}
...
{ 
	...
	void surf (Input IN, inout SurfaceOutput o)
	{
		...
		float mask = 1-AOtex.b;
		o.Normal = (float3 (normal01.rg + normal02.rg ,1.0) * mask) + (normal03 * AOtex.b);
	    ...
	    o.Albedo = (tex.rgb * AOtex.r * AODtex.g * mask) + (sectex * AOtex.r * AOtex.b) ;	
	}
	ENDCG
}
...
{ 
	...
	void surf (Input IN, inout SurfaceOutput o)
	{
		...
		fixed4 sectex = tex2D(_SecTex, IN.uv_SecTex);
		fixed4 AOtex = tex2D(_AOTex, IN.uv_AOTex);
		fixed4 AODtex = tex2D(_AOTex, IN.uv_AOTex);
		
		float3 normal01 = UnpackNormal(tex2D(_BumpMap, IN.uv_BumpMap));
		float3 normal02 = UnpackNormal(tex2D(_SecBumpMap, IN.uv_SecTex));
		
		float mask = 1-AOtex.b;
		o.Normal = (float3 (normal01.rg + normal02.rg ,1.0) * mask);
		//o.Normal = float3 (normal01.rg + normal02.rg ,1.0);
		o.Gloss = tex.a;
		o.Alpha = tex.a;
		_SpecColor *= AOtex.r;
